...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Getting input values and sanitizing them
    $name = isset($_POST['name']) ? htmlspecialchars($_POST['name']) : '';
    $email = isset($_POST['email']) ? htmlspecialchars($_POST['email']) : '';
    $password = isset($_POST['password']) ? htmlspecialchars($_POST['password']) : '';

    // Preparing a query to select the user with matching credentials
    ...
    if ($res->num_rows > 0) {
        $row = $res->fetch_assoc();
        $id = $row['ID'];
        // Redirecting to index.php with the user ID
        header("Location: index.php?id=" . urlencode($id));
        exit();
    } else {
        // If no user found, redirect with error
        header("Location: login.php?error=No+Users+found");
        exit();
    }
...
<body>
    <div class="container">
        ...
        <form action="login.php" method="post">
            ...
            <center>
                <a href="signin.php" class="btn1">Sign Up</a>
            </center>
        </form>
    </div>
